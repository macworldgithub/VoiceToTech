...
// Backend endpoint that transcribes audio, extracts fields, stores any
// attached photos/videos, and saves the draft record to MongoDB.
const TRANSCRIBE_URL = "https://vttbackend.omnisuiteai.com/transcribe";
...
// Accept as many photo/video formats as possible — including iPhone's
// HEIC/HEIF photos and MOV/HEVC videos — by accepting broad MIME categories
// plus explicit extensions for formats some browsers don't map cleanly.
const PHOTO_ACCEPT =
  "image/*,.heic,.heif,.heics,.heifs,.jpg,.jpeg,.png,.webp,.gif,.bmp,.tiff";
const VIDEO_ACCEPT =
  "video/*,.mov,.m4v,.mp4,.3gp,.3g2,.avi,.mkv,.wmv,.webm,.hevc";
...
function App() {
  ...
  // ---- Photo / video capture -------------------------------------------
  // Kind is tracked explicitly based on which button the tech pressed,
  // rather than guessed from file extension/MIME type. This is what makes
  // iPhone HEIC photos, MOV videos, and any other format work reliably —
  // there's never any extension-sniffing to get wrong.
  ...
  // Some browsers (notably Chrome/Android) can't render HEIC previews
  // inline. Safari/iOS renders them fine. We fall back to a simple file
  // icon/label instead of a broken image so it never looks like an error.
  ...
  const submitRecording = async () => {
    ...
    // Attach photos and videos under separate field names so the backend
    // knows the kind directly — no extension guessing needed there either.
    media.forEach((item) => {
      if (item.kind === "image") {
        formData.append("photos", item.file, item.file.name);
      } else {
        formData.append("videos", item.file, item.file.name);
      }
    });
    ...
    try {
      ...
      // result.Media is an array of { url, type } objects
      const mediaItems = Array.isArray(result["Media"]) ? result["Media"] : [];
      const orderId = result["_id"] || "";

      router.push(
        `/vehicleinfo?id=${encodeURIComponent(orderId)}&workNumber=${encodeURIComponent(
          result["Work Number"] || ""
        )}&vehicleInfo=${encodeURIComponent(
          result["Vehicle Information"] || ""
        )}&problem=${encodeURIComponent(
          result["Problem Description"] || ""
        )}&media=${encodeURIComponent(JSON.stringify(mediaItems))}`
      );
    } catch (err) {
      console.error("Upload error:", err);
      alert("Failed to submit recording. Please try again.");
                  ...
                  {media.length > 0 && (
                    <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 sm:gap-2.5 mt-3 sm:mt-4">
                      {media.map((item, index) => (
                        ...
                        >
                          {item.kind === "image" ? (
                            isLikelyHeic(item.file) ? (
                              <div className="w-full h-full flex flex-col items-center justify-center gap-1 text-white/40 px-1 text-center">
                                <ImagePlus className="w-5 h-5" />
